...
int main(void)
{
	...
	{	
		...
		if (digit_num%2!=0) // if the last digit is odd, implement the f function and add the result to check_sum
			check_sum += ((2*digit)%10 + (2*digit)/10); // Update the check_sum
		else // if the last digit is even, add the last digit to check_sum directly
			check_sum += digit; // // Update the check_sum
		
		account_number = account_number/10; // Discard the last digit and update the account number with remaining part
		digit_num -=1; // Decrease the digit number
	}
